...
class SearchItemDisplayWidget(BaseClickableFrame):
    ...
    def __init__(
        ...
        title_widget = QLabel(f"{filter_def.title}:", self)

        value_wrapper = QWidget(self)
        value_wrapper.setAttribute(Qt.WA_TranslucentBackground, True)
        value_widget = QLabel(value_wrapper)
        value_widget.setObjectName("ValueWidget")
        value_widget.setText("")
        value_layout = QVBoxLayout(value_wrapper)
        ...
        main_layout.setContentsMargins(8, 4, 4, 4)
...
class FilterItemButton(BaseClickableFrame):
    ...
    def __init__(
        ...
        title_widget = QLabel(filter_def.title, self)
...
class FiltersPopup(QWidget):
    filter_requested = Signal(str)
    text_filter_requested = Signal(str)
    ...
    def __init__(self, parent):
        ...
        wraper_layout = QVBoxLayout(wrapper)
...
class FilterValueItemButton(BaseClickableFrame):
    ...
    def __init__(self, widget_id, value, icon, color, parent):
        ...
        title_widget = QLabel(str(value), self)
        if color:
...
class FilterValueTextInput(QWidget):
    ...
    def __init__(self, parent):
        ...
        btns_widget.setAttribute(Qt.WA_TranslucentBackground, True)

        text_input = QLineEdit(self)

        back_btn = QPushButton("← Back", btns_widget)
        back_btn.setObjectName("BackButton")
        confirm_btn = QPushButton("Confirm", btns_widget)
...
class FilterValueItemsView(QWidget):
    ...
    def __init__(self, parent):
        ...
        select_all_btn = QPushButton("Select all", btns_widget)
        clear_btn = QPushButton("Clear", btns_widget)
        swap_btn = QPushButton("Invert", btns_widget)
...
class FilterValuePopup(QWidget):
    value_changed = Signal(str)
    closed = Signal(str)
...
class FiltersBar(BaseClickableFrame):
    ...
    def __init__(self, parent):
        super().__init__(parent)
        # Height will be set by the controller for consistency
        ...
        filters_wrap = QWidget(self)
        filters_wrap.setAttribute(Qt.WA_TranslucentBackground, True)

        # Placeholder label when no filters are active
        placeholder_label = QLabel("Click 🔍 to add filters...", filters_wrap)
        placeholder_label.setObjectName("PlaceholderLabel")
        placeholder_label.setStyleSheet("color: #888888; font-style: italic; padding: 8px;")

        # Widget where set filters are displayed
        filters_widget = QWidget(filters_wrap)
        filters_widget.setAttribute(Qt.WA_TranslucentBackground, True)
    ...
    def _update_placeholder_visibility(self):
        """Show/hide placeholder based on active filters."""
        has_filters = len(self._widgets_by_name) > 0
        self._placeholder_label.setVisible(not has_filters)
        self._filters_widget.setVisible(has_filters)

        if not has_filters:
            # Position placeholder in the center of the wrap area
            wrap_rect = self._filters_wrap.rect()
            label_size = self._placeholder_label.sizeHint()
            x = max(0, (wrap_rect.width() - label_size.width()) // 2)
            y = max(0, (wrap_rect.height() - label_size.height()) // 2)
            self._placeholder_label.move(x, y)
    ...
    def _show_popup(self, popup: QWidget):
        """Show a popup widget."""
        geo = self.geometry()
        bl_pos_g = self.mapToGlobal(QPoint(0, geo.height() + 5))
        popup.show()
